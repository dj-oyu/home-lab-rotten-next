...
### コードサイズの変化
```
変更前の実質コード: 約70行（exploit-official-poc.js）+ 約25行（exploit-sample.js）
変更後の実質コード: 約40行（exploit-official-poc.js）+ 約30行（exploit-sample.js）
変更後のコメント: 約25行（exploit-sample.js内のドキュメント）

削減: 約25行のボイラープレートコード（ストリーム処理など）
追加: 約25行の教育的コメント（エクスプロイト解説）
```
